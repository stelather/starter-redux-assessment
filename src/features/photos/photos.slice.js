...
const options = {
  ...
  reducers: {
    ...
    addPhoto: (initialState , action) => {
      initialState.photos.unshift(action.payload);
    },

    removePhoto: (initialState, action) => {
      initialState.photos.splice(initialState.photos.findIndex((photo) => photo.id === action.payload),1 );
    },
